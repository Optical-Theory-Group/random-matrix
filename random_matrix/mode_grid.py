...
from .utils.geometry_utils import polar_to_cartesian
...
class ModeGrid:
    def __init__(self, grid_data: dict[str, Any] | None = None) -> None:
        ...
        match grid_data:
            ...
            case {"grid_type": "cartesian", "dx": dx, "dy": dy}:
                self._handle_cartesian_case(dx=dx, dy=dy)
        """
    ...
    def _handle_cartesian_case(self, dx: float, dy: float, x_lim: float = 1.0,
                               y_lim: float = 1.0) -> None:
        # Set up x-y lattice
