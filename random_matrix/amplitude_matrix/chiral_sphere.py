...
def get_A_from_mus(
    mus: FloatLike, xs: FloatLike, ms: FloatLike, mrs: FloatLike, mls: FloatLike
) -> FloatLike:
    ...
    djmrx = (
        ((mrs * xs) ** 2.0 - 2.0) * np.sin(mrs * xs) + 2.0 * mrs * xs * np.cos(mrs * xs)
    ) / (mrs * xs) ** 3
    djmlx = (
        ((mls * xs) ** 2.0 - 2.0) * np.sin(mls * xs) + 2.0 * mls * xs * np.cos(mls * xs)
    ) / (mls * xs) ** 3
    ...
    for n in range(2, num_stop + 1):
        ...
        S_3 = S_3 + (2.0 * n + 1.0) / (n * (n + 1.0)) * (c * (new_tau + b * new_pi))
        S_4 = -S_3
...
def get_A(
    ...
    A_scattering_plane = np.transpose(
        get_A_from_mus(mus, xs, ms, mrs, mls).reshape(2, 2, num_one, num_two),
        (2, 3, 0, 1),
    )
