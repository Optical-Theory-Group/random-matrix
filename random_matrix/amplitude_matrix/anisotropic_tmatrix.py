...
class AmatrixGenerator:
    ...
    def MN_lmn(k, l, m, n, R, T, P):
        ...
        if l == 3:  # bessel function - non -diverging multipoles
            ...
            zldot = (1 + n) * zl / (k * R) - np.sqrt(np.pi / (2 * k * R)) * jv(
                n + 1 + 1 / 2, k * R
            )
        ...
        N_lmn_sp = np.array(
            [(zl / (k * R)) * p_mn[kk] + zldot * n_mn[kk] for kk in range(3)]
        )
    ...
    def MN_orthogonality_matrix(k, nmax, R, Nt, Np):
        ...
        (
            ...
            psurf_weights,
            psurf_norms,
        ) = AmatrixGenerator.define_sphere(R, Nt, Np)
        ...
        for jj in range(1, nummodes):  # don't include multipole
            ...
            AmatrixGenerator.update_progress(
                jj / nummodes,
                "Pre-calculating modes: (n,m) = ({},{})".format(n, m),
            )
        ...
        for jj1 in range(1, nummodes):
            ...
            jj1minusm = AmatrixGenerator.lm_2_n_all(
                n1, -m1
            )  # since projections required -m modes
            ...
            m1m1_anal[jj1, jj1] = (
                ...
                * n1
                * (n1 + 1)
                * factorial(n1 + np.abs(m1))
                / ((2 * n1 + 1) * factorial(n1 - np.abs(m1)))
            )
            m3m3_anal[jj1, jj1] = (
                zn3**2
                * 4
            ...
            n1n1_anal[jj1, jj1] = (
                zn1 * n1 * (n1 + 1) / (k * R)
            ) ** 2 * 4 * np.pi * factorial(n1 + np.abs(m1)) / (
                (2 * n1 + 1) * factorial(n1 - np.abs(m1))
            ) + zn1dot**2 * n1 * (
                n1 + 1
            ) * 4 * np.pi * factorial(
            ...
            for jj2 in range(1, nummodes):  # -n to n
                ...
                m1m1[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", M1_mn[jj2], M1_mn[jj1minusm])
                    * psurf_weights
                )
                n1m1[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", N1_mn[jj2], M1_mn[jj1minusm])
                    * psurf_weights
                )
                m1n1[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", M1_mn[jj2], N1_mn[jj1minusm])
                    * psurf_weights
                )
                n1n1[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", N1_mn[jj2], N1_mn[jj1minusm])
                    * psurf_weights
                )

                m3m3[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", M3_mn[jj2], M3_mn[jj1minusm])
                    * psurf_weights
                )
                n3m3[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", N3_mn[jj2], M3_mn[jj1minusm])
                    * psurf_weights
                )
                m3n3[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", M3_mn[jj2], N3_mn[jj1minusm])
                    * psurf_weights
                )
                n3n3[jj1, jj2] = np.sum(
                    np.einsum("j...,j...->...", N3_mn[jj2], N3_mn[jj1minusm])
                    * psurf_weights
                )

        Q1 = np.concatenate(
            (
                np.concatenate((m1m1[1:, 1:], n1m1[1:, 1:]), axis=0),
                np.concatenate((m1n1[1:, 1:], n1n1[1:, 1:]), axis=0),
            ),
            axis=1,
        )
        Q3 = np.concatenate(
            (
                np.concatenate((m3m3[1:, 1:], n3m3[1:, 1:]), axis=0),
                np.concatenate((m3n3[1:, 1:], n3n3[1:, 1:]), axis=0),
            ),
            axis=1,
        )

        Q1_anal = np.concatenate(
            (
                np.concatenate((m1m1_anal[1:, 1:], n1m1_anal[1:, 1:]), axis=0),
                np.concatenate((m1n1_anal[1:, 1:], n1n1_anal[1:, 1:]), axis=0),
            ),
            axis=1,
        )
        Q3_anal = np.concatenate(
            (
                np.concatenate((m3m3_anal[1:, 1:], n3m3_anal[1:, 1:]), axis=0),
                np.concatenate((m3n3_anal[1:, 1:], n3n3_anal[1:, 1:]), axis=0),
            ),
            axis=1,
        )
    ...
    def XYeh_mn(omega, eps, mu, m, n, T, P, W, psurf_coords):
        ...
        Xe_mn = np.array(
            [np.zeros(Xs.shape), np.zeros(Xs.shape), np.zeros(Xs.shape)],
            dtype=complex,
        )
        Ye_mn = np.array(
            [np.zeros(Xs.shape), np.zeros(Xs.shape), np.zeros(Xs.shape)],
        ...
        lrp = Lambda[0, 2]
        prefac = (
            1 / (1 - f1 * f2) * W * np.sin(T)
        )  # include integration weights and Jacobian

        # eigenpolarisation wavenumbers and wavevectors in cartesian basis
        k1 = omega * np.sqrt(mu / l12[0])
        k2 = omega * np.sqrt(mu / l12[1])
        k1vc = AmatrixGenerator.sph2cart_comp_vec(
            np.array([k1, np.zeros(T.shape), np.zeros(T.shape)]), T, P
        )
        k2vc = AmatrixGenerator.sph2cart_comp_vec(
            np.array([k2, np.zeros(T.shape), np.zeros(T.shape)]), T, P
        )

        # construct w polarisation vectors in spherical basis
        w1e = l12[0] * v12[0] + np.array(
            [lrt * f1 + lrp, np.zeros(T.shape), np.zeros(T.shape)]
        )
    ...
    def d2(bta):
        d2 = np.array(
            [
                [
                    (1 + np.cos(bta)) ** 2 / 4,
                    -(1 + np.cos(bta)) * np.sin(bta) / 2,
                    np.sqrt(6) * (np.sin(bta)) ** 2 / 4,
                    -(1 - np.cos(bta)) * np.sin(bta) / 2,
                    (1 - np.cos(bta)) ** 2 / 4,
                ],
                [
                    (1 + np.cos(bta)) * np.sin(bta) / 2,
                    (1 + np.cos(bta)) * (2 * np.cos(bta) - 1) / 2,
                    -np.sqrt(3 / 2) * np.cos(bta) * np.sin(bta),
                    (1 - np.cos(bta)) * (2 * np.cos(bta) + 1) / 2,
                    -(1 - np.cos(bta)) * np.sin(bta) / 2,
                ],
                [
                    np.sqrt(6) * (np.sin(bta)) ** 2 / 4,
                    np.sqrt(3 / 2) * np.sin(bta) * np.cos(bta),
                    3 * (np.cos(bta)) ** 2 / 2 - 1 / 2,
                    -np.sqrt(3 / 2) * np.cos(bta) * np.sin(bta),
                    np.sqrt(6) * (np.sin(bta)) ** 2 / 4,
                ],
                [
    ...
    def WignerD_rot_mat(alpha, beta, gamma, lmin, lmax):
        ninds = range(
            AmatrixGenerator.lm_2_n_all(lmin, -lmin),
            AmatrixGenerator.lm_2_n_all(lmax, lmax) + 1,
        )
    ...
    def calc_Q1Q3(
        ...
        nummodes = AmatrixGenerator.lm_2_n_all(nmax, nmax) + 1
        Xe_mn = np.squeeze(
            np.zeros((nummodes, 3) + psurf_coords[0].shape, dtype=complex)
        )
        Ye_mn = np.squeeze(
            np.zeros((nummodes, 3) + psurf_coords[0].shape, dtype=complex)
        )
        ...
        for jj1 in range(1, nummodes):
            ...
            for jj2 in range(1, nummodes):  # -n to n
                ...
                # update_progress((nummodes*jj1+jj2)/nummodes**2, "Calculating Q overlap integrals")
                q1_11kern = fdg1 * np.einsum(
                    "j...,j...->...",
                    np.cross(psurf_norms, Xh_mn[jj2], axis=0),
                    M1_mn[jj1minusm],
                ) - 1j * ksoverwmu * np.einsum(
                    "j...,j...->...",
                    np.cross(psurf_norms, Xe_mn[jj2], axis=0),
                    N1_mn[jj1minusm],
                )
                q1_12kern = fdg1 * np.einsum(
                    "j...,j...->...",
                    np.cross(psurf_norms, Yh_mn[jj2], axis=0),
                    M1_mn[jj1minusm],
                ) - 1j * ksoverwmu * np.einsum(
                    "j...,j...->...",
                    np.cross(psurf_norms, Ye_mn[jj2], axis=0),
                    N1_mn[jj1minusm],
                )
                q1_21kern = fdg1 * np.einsum(
                    "j...,j...->...",
                    np.cross(psurf_norms, Xh_mn[jj2], axis=0),
                    N1_mn[jj1minusm],
                ) - 1j * ksoverwmu * np.einsum(
        ...
        Q1 = np.concatenate(
            (
                np.concatenate((Q1_11[1:, 1:], Q1_21[1:, 1:]), axis=0),
                np.concatenate((Q1_12[1:, 1:], Q1_22[1:, 1:]), axis=0),
            ),
            axis=1,
        )
        Q3 = np.concatenate(
            (
                np.concatenate((Q3_11[1:, 1:], Q3_21[1:, 1:]), axis=0),
                np.concatenate((Q3_12[1:, 1:], Q3_22[1:, 1:]), axis=0),
            ),
            axis=1,
        )
        return Q1, Q3
    ...
    def calc_Tmatrix(params):
        ...
        Q1, Q3 = AmatrixGenerator.calc_Q1Q3(
            nmax,
            omega,
            eps_s,
            eps_p,
        ...
        if rot_avg:
            ...
            RTMAT = AmatrixGenerator.average_Tmatrix(
                Tmat, nmax, alpha, beta, gamma, pdf
            )
        else:
            alpha = params["euler_alpha"]
            beta = params["euler_beta"]
    ...
    def driccati_psi_n(n, z):
        return np.sqrt(np.pi / (2 * z)) * (n + 1) * jv(n + 1 / 2, z) - np.sqrt(
            np.pi * z / 2
        ) * jv(n + 3 / 2, z)
    ...
    def calc_eta_zeta_mn(n, k1, k2, r):
        ...
        an = (
            m
            * AmatrixGenerator.riccati_psi_n(n, z2)
            * AmatrixGenerator.driccati_psi_n(n, z1)
            - AmatrixGenerator.driccati_psi_n(n, z2)
            * AmatrixGenerator.riccati_psi_n(n, z1)
        ) / (
            m
            * AmatrixGenerator.riccati_psi_n(n, z2)
            * AmatrixGenerator.driccati_xi_n(n, z1)
            - AmatrixGenerator.driccati_psi_n(n, z2)
            * AmatrixGenerator.riccati_xi_n(n, z1)
        )
        bn = (
            m
            * AmatrixGenerator.driccati_psi_n(n, z2)
    ...
    def calc_Tmatrix_sphere(nmax, k1, k2, r):
        ...
        T = np.concatenate(
            (
                np.concatenate((Z[1:, 1:], 0 * N[1:, 1:]), axis=0),
                np.concatenate((0 * Z[1:, 1:], N[1:, 1:]), axis=0),
            ),
            axis=1,
        )
    ...
    def calc_Amat_from_Tmat(k_s, T, ninc, nsca):
        ...
        dimt = T.shape[0]
        nt = int(dimt / 2)
        nmax, _ = AmatrixGenerator.n_2_lm_all(
            nt
        )  # add one since we assume monopole has been removed
        nummodes = AmatrixGenerator.lm_2_n_all(nmax, nmax) + 1
        ...
        for jji in range(1, nummodes):
            ...
            mmarray[jji] = mi
            prefacs[jji] = np.sqrt(
                factorial(ni - np.abs(mi)) / factorial(ni + np.abs(mi))
            )
        ...
        for jji in range(1, nummodes):
            ...
            for jjs in range(1, nummodes):
                ...
                s11 += (
                    alphamnmn
                    * (
                        T11mnmn * pi_sca * pi_inc
                        + T21mnmn * tau_sca * pi_inc
                        + T12mnmn * pi_sca * tau_inc
                        + T22mnmn * tau_sca * tau_inc
                    )
                    * exp_sca
                    * exp_inc
                )
                s12 += (
                    ...
                    * alphamnmn
                    * (
                        T11mnmn * pi_sca * tau_inc
                        + T21mnmn * tau_sca * tau_inc
                        + T12mnmn * pi_sca * pi_inc
                        + T22mnmn * tau_sca * pi_inc
                    )
                    * exp_sca
                    * exp_inc
                )
                s21 += (
                    ...
                    * alphamnmn
                    * (
                        T11mnmn * tau_sca * pi_inc
                        + T21mnmn * pi_sca * pi_inc
                        + T12mnmn * tau_sca * tau_inc
                        + T22mnmn * pi_sca * tau_inc
                    )
                    * exp_sca
                    * exp_inc
                )
                s22 += (
                    alphamnmn
                    * (
                        T11mnmn * tau_sca * tau_inc
                        + T21mnmn * pi_sca * tau_inc
                        + T12mnmn * tau_sca * pi_inc
                        + T22mnmn * pi_sca * pi_inc
                    )
                    * exp_sca
                    * exp_inc
                )
        ...
        if "particle_type" in params.keys():
            ...
            if hasattr(self, "tmatrix"):
                ...
                if params == self.params:
                    ...
                    Amat = self.calc_Amat_from_Tmat(
                        self.params["k_s"], self.tmatrix, k1, k2
                    )
                    return Amat
                else:
                    print(
                        "Input particle parameters have changed. Reinitialising Tmatrix..."
                    )
