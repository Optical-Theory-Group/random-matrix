...
analysis_functions = {"test": test}
num_matrices = 10**4
data = pm.cascade(
    num_matrices, [3], analysis_functions, use_transfer_matrices=False
)
pm.cascade_hdf5(
    "test",
    num_matrices,
...
f = h5py.File(
    "/home/nbyrnes/code/random-matrix/data/hdf5_test/cascades/test", "r+"
)

print(np.abs(np.mean(data["test"][0])))
print(np.abs(np.mean(f["test"][0])))
f.close()
