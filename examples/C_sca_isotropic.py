...
def an_bn(m,x):
    ...
    xix = psix + 1j * phix
    dxix = dpsix + 1j * dphix  
    a = (m * psimx * dpsix - psix * dpsimx) / (
        m * psimx * dxix - xix * dpsimx
    )
    b = (psimx * dpsix - m * psix * dpsimx) / (
        psimx * dxix - m * xix * dpsimx
    )
...
Q_ext1 = np.zeros(1000);Q_ext2 = np.zeros(1000)

for i in range(0,1000):
    Q_ext1[i] = (an_bn(m,size_param[i])*((2*np.pi)/k1**2))/(np.pi*radius1[i]**2)
