...
Ts_xstretch = []
r = x / k1
n_max1 = 2#int(np.max(np.floor(x + 4.05 * x**0.33333 + 2.0) + 1))
n_max2 = 4
x = [0.001,0.01,0.1,1,2]
Ts_n2 = []
Ts_n1 = []
for size in tqdm.tqdm(x):
    r = size / k1
    hull_sphere = isotropic_tmatrix.ellipse_hull(r, r, r)
    T_sphere_n2 = isotropic_tmatrix.get_T(hull_sphere, k1, k2, n_max2)
    T_sphere_n1 = isotropic_tmatrix.get_T(hull_sphere, k1, k1, n_max1)
    Ts_n2.append(T_sphere_n2)
    Ts_n1.append(T_sphere_n1)

output_file = "Ts_n.pkl"
results = {
    "Ts_n2": Ts_n2,
    "Ts_n1": Ts_n1,
    "size": x,
