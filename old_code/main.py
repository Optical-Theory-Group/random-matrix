...
#         data[block_str]["Matrices"] = np.zeros(
#             (n_times, 2, 2), dtype=np.complex128
#         )
...
def main():
    ...
    sigma_p = np.zeros(
        (4 * on_axis_index + 2, 4 * on_axis_index + 2), dtype=np.complex128
    )
    ...
    for params in params_array:
        ...
        # n_mat_array = np.array(
        #     [1]
        #     + [
        #         i * mat_spacing
        #         for i in range(1, int(np.round(L_final / L_spacing)))
        #     ]
        # )
        ...
        # exponential_list = np.array(
        #     [np.exp(1j * k * kz * dL) for kz in kz_list]
        # )
        # lambda_plus = np.kron(np.diag(exponential_list), np.identity(2))
        # lambda_minus = np.conj(lambda_plus.T)
        # lambda_plus_minus = np.block(
        #     [
        #         [lambda_plus, np.zeros((2 * n_modes, 2 * n_modes))],
        #         [np.zeros((2 * n_modes, 2 * n_modes)), lambda_minus],
        #     ]
        # )
        ...
        #     _ = matrix_group.create_dataset(
        #         "Single Pool M",
        #         (n_single_pool, mat_size, mat_size),
        #         dtype=np.complex128,
        #     )
        #     _ = matrix_group.create_dataset(
        #         "Single Pool S",
        ...
        #     _ = matrix_group.create_dataset(
        #         "Multi Pool S",
        #         (n_multi_pool, mat_size, mat_size),
        #         dtype=np.complex128,
        #     )
        #     _ = matrix_group.create_dataset(
        #         "Multi Pool M",
        ...
        #     )
        #     _ = matrix_group.create_dataset(
        #         "Matrices", (n_times, mat_size, mat_size), dtype=np.complex128
        #     )
        ...
        #     _ = data_group.create_dataset(
        #         "Thicknesses", (n_steps,), dtype=float, data=L_array
        #     )
        #     _ = data_group.create_dataset(
        #         "n_matrices", (n_steps,), dtype=int, data=n_mat_array
        #     )
        #     _ = data_group.create_dataset(
        #         "Transmission", (n_steps, n_times), dtype=float
        #     )
        #     _ = data_group.create_dataset(
        #         "Tau", (n_steps, n_times * 2 * n_modes), dtype=np.complex128
        #     )

        #     for block in data_blocks:
        #         data_subgroup = data_group.create_group(str(block))
        #         data_subgroup["Mode"] = str(block)
        #         _ = data_subgroup.create_dataset(
        #             "Diattenuation", (n_steps, n_times), dtype=float
        #         )
        #         _ = data_subgroup.create_dataset(
        #             "Retardance", (n_steps, n_times), dtype=float
        #         )
        #         _ = data_subgroup.create_dataset(
        #             "Mean", (n_steps, 2, 2), dtype=np.complex128
        #         )
        #         _ = data_subgroup.create_dataset(
        #             "H", (n_steps, 4, 4), dtype=np.complex128
        #         )
        #         _ = data_subgroup.create_dataset(
        #             "Matrices", (n_steps, n_times, 2, 2), dtype=np.complex128
        #         )
        ...
        #         if p + 1 in [
        #             int(n_single_pool * n / 10) for n in range(1, 11)
        #         ]:
        ...
        #     for p in range(n_times):
        #         if cascade_method == "M":
        #             matrix_group["Matrices"][p, :, :] = np.identity(
        #                 mat_size, dtype=np.complex128
        #             )
        #         elif cascade_method == "S":
        #             matrix_group["Matrices"][p, :, :] = np.block(
        #                 [
        #                     [
        #                         np.zeros(
        #                             (2 * n_modes, 2 * n_modes),
        #                             dtype=np.complex128,
        #                         ),
        #                         np.identity(2 * n_modes, dtype=np.complex128),
        #                     ],
        #                     [
        #                         np.identity(2 * n_modes, dtype=np.complex128),
        #                         np.zeros(
        ...
        #                     matrices[p, :, :] = S_cascade(
        #                         S_new, matrices[p, :, :], sigma_p
        #                     )
        ...
        #                     if (
        #                         not switch
        #                         and np.max(np.abs(matrices[p, :, :])) > 10**6
        #                     ):
        #                         print("Transfer matrix elements too large...")
        #                         print(
        #                             "Maximum value = ",
        #                             np.max(np.abs(matrices[p, :, :])),
        #                         )
        #                         switch = True
        ...
        #         data_group = f["Data"]
        #         data_group["Transmission"][thickness_index] = new_data[
        #             "Transmission"
        #         ]
        ...
        #             mode_str = str(mode)
        #             data_group[mode_str]["Diattenuation"][
        #                 thickness_index, :
        #             ] = new_data[mode_str]["Diattenuation"]
        #             data_group[mode_str]["Retardance"][
        #                 thickness_index, :
        #             ] = new_data[mode_str]["Retardance"]
        #             data_group[mode_str]["Mean"][
        #                 thickness_index, :, :
        #             ] = new_data[mode_str]["Mean"]
        #             data_group[mode_str]["H"][
        #                 thickness_index, :, :
        #             ] = new_data[mode_str]["H"]
        #             data_group[mode_str]["Matrices"][
        #                 thickness_index, :, :, :
        #             ] = new_data[mode_str]["Matrices"]
